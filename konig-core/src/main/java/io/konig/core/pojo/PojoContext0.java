...
public class PojoContext0 {
	...
	private Map<Class<?>, PojoCreator<?>> creatorMap;
	...
	public PojoContext0(PojoContext0 copy) {
		classMap = copy.classMap;
		deserializerMap = copy.deserializerMap;
		individualMap = new LinkedHashMap<>();
	}

	public PojoContext0(Graph ontology) {
		this.ontology = ontology;
	}
	
	public void putCreator(Class<?> type, PojoCreator<?> creator) {
		if (creatorMap == null) {
	...
	public PojoCreator<?> getCreator(Class<?> type) {
		return creatorMap == null ? null : creatorMap.get(type);
	}
	...
	public PojoHandler getPojoHandler(Class<?> javaClass) {
		return pojoHandlerMap==null ? null : pojoHandlerMap.get(javaClass);
	}
