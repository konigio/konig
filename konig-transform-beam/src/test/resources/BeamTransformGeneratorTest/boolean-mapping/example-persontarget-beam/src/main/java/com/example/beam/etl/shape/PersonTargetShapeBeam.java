...
public class PersonTargetShapeBeam {
    ...
    public static void process(PersonTargetShapeBeam.Options options) {
        ...
        outputTuple.get(ReadPersonSourceShapeFn.deadLetterTag).setCoder(NullableCoder.of(StringUtf8Coder.of())).apply("writeErrorDocument", org.apache.beam.sdk.io.FileIO.<String, String> writeDynamic().via(TextIO.sink()).by(new SerializableFunction() {
        ...
        ).to(MessageFormat.format(pattern, "application/vnd.example.ns.shape.person")).withNumShards(1).withDestinationCoder(NullableCoder.of(StringUtf8Coder.of())).withNaming(key -> org.apache.beam.sdk.io.FileIO.Write.defaultNaming(("file-"+ key), ".csv")));
        ...
        outputTuple2 .get(ToPersonTargetShapeFn.deadLetterTag).setCoder(NullableCoder.of(StringUtf8Coder.of())).apply("writeErrorDocument", org.apache.beam.sdk.io.FileIO.<String, String> writeDynamic().via(TextIO.sink()).by(new SerializableFunction() {
        ...
        ).to(MessageFormat.format(pattern, "application/vnd.example.ns.shape.person")).withNumShards(1).withDestinationCoder(NullableCoder.of(StringUtf8Coder.of())).withNaming(key -> org.apache.beam.sdk.io.FileIO.Write.defaultNaming(("file-"+ key), ".txt")));
        p.run();
    }
