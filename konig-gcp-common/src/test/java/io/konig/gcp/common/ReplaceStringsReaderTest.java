...
public class ReplaceStringsReaderTest {
	
	@Test
	public void testPartialArrayLarge() throws Exception {
		try (
				StringReader source = new StringReader("one foofoo two foo three");
				ReplaceStringsReader reader = new ReplaceStringsReader(source, "foo", "alpha", "two", "beta");
			) {
				...
				assertEquals("one alphaalpha beta alpha three", text);
	...
	public void testPartialArraySmall() throws Exception {
		try (
				...
				ReplaceStringsReader reader = new ReplaceStringsReader(source, "alpha", "foo", "two", "beta");
			) {
				StringBuilder builder = new StringBuilder();
				int len;
				char[] buf = new char[7];
				...
				String text = builder.toString();
				assertEquals("one foofoo beta foo three", text);
