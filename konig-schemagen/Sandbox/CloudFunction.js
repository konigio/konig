...
exports.triggerDag = function triggerDag(event) {
  ...
  const PROJECT_ID = ${gcpProjectId}; // 'sam-edw';
  // Navigate to your webserver's login page and get this from the URL
  const CLIENT_ID = ${clientId}; //'501598107182-4tnsfg4m0rr3nsnk2hdqdc04i7g2o4n7.apps.googleusercontent.com';
  // This should be part of your webserver's URL:
  // {tenant-project-id}.appspot.com
  const WEBSERVER_ID = ${webServerId}; //'f9a8a604ec2ec9326-tp';
  // The name of the DAG you wish to trigger
  const DAG_NAME = ${dagName}; //'GcsToBigQueryTriggered';

  // Other constants
  const WEBSERVER_URL = `https://${webServerId}.appspot.com/api/experimental/dags/${dagName}/dag_runs`;
  const USER_AGENT = 'gcf-event-trigger';
  const BODY = {conf: JSON.stringify(event.data)};

  // Make the request
  return authorizeIap('${clientId}', '${gcpProjectId}', USER_AGENT).then(
