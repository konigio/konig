...
import io.konig.camel.aws.s3.DeleteObjectEndpoint;

/**
 * The DeleteObject consumer.
 */
public class DeleteObjectConsumer extends ScheduledBatchPollingConsumer {
	
	private static final Logger LOG = LoggerFactory.getLogger(DeleteObjectConsumer.class);
    private String marker;
    private transient String deleteObjectConsumerToString;

    public DeleteObjectConsumer(DeleteObjectEndpoint endpoint, Processor processor) throws NoFactoryAvailableException {
    ...
    @Override
    public DeleteObjectEndpoint getEndpoint() {
        return (DeleteObjectEndpoint) super.getEndpoint();
    }

    @Override
    public String toString() {
        if (deleteObjectConsumerToString == null) {
            deleteObjectConsumerToString = "S3Consumer[" + URISupport.sanitizeUri(getEndpoint().getEndpointUri()) + "]";
        }
        return deleteObjectConsumerToString;
    }
}
